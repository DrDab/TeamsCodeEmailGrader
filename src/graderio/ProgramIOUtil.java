...
public class ProgramIOUtil 
{
	...
	public PostExecutionResults runExecutable(File toRun, List<String> args, String stdin, String logName, long timeoutMs) throws InterruptedException
	{
		...
		File programOutputFile = toRun.getParent() == null ? new File(logName + "-program-output") : new File(toRun.getParent(), logName + "-program-output");
		File programErrFile =  toRun.getParent() == null ? new File(logName + "-program-error") : new File(toRun.getParent(), logName + "-program-error");
		try 
		{
		    ...
		    {
		    	return new PostExecutionResults(stdin, stdout, stderr, timeTaken, ExecutionResultStatus.FAILED_TIMED_OUT);
		    }
		} 
		catch (IOException e)
		{
			...
			e.printStackTrace();
			return new PostExecutionResults(e.getMessage(), e.getMessage(), e.getMessage(), 0.0, ExecutionResultStatus.FAILED_COMMAND_ERR);
		}
	}
